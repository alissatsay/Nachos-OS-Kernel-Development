...
public class UserKernel extends ThreadedKernel {
    ...
    */
    public static int[] allocateFrames(int requested) {
        if (requested <= 0) return new int[0];

        Object[] objs = freeFrames.removeFirstN(requested);
        if (objs == null) return null;

        int[] out = new int[requested];
        for (int i = 0; i < requested; i++) {
            out[i] = ((Integer) objs[i]).intValue();
        }
        return out;
    }
    ...
    public static void releaseFrame(int frameNumber) {
        if (frameNumber < 0 || frameNumber >= numPhysPages) return;
        freeFrames.insert(frameNumber, frameNumber);
    }
